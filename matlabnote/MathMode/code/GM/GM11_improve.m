function gm11 = GM11_improve(X,td)

%GM11_improve基于背景值优化的改进GM(1,1)模型建立和预测  
...
Z = (Ago(1:n-1)- Ago(2:end))./(log(Ago(1:n-1))-log(Ago(2:end)));
 %%构造B和Ynz矩阵
...
PreData = [Ago(1),(1-exp(a))*(Ago(1)-u/a)*exp(-a*(1:n+td-1))];
 %% 数据可视化
 t = 1:n;
