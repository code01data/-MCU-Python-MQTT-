% 使用Fun_ARIMA_Forecast_MulComp函数实现预测的公开版代码
% 划分训练集、测试集进行使用ARIMA进行“多步”预测，并进行预测效果对比
...
%% 2.调用函数并实现单步预测
TrainR = 0.85; %用于训练的数据长度，也可以写成小于1的小数作为比例值
max_ar = 3;  %p上限值
max_ma = 3;  %q上限值
[forData,dataTrain,dataTest,aicorbic,res] = Fun_ARIMA_Forecast_MulComp(data,TrainR,max_ar,max_ma,'on','aic+bic');
