% 使用Fun_ARIMA_Forecast_Onestep函数实现预测的公开版代码
% 划分训练集、测试集进行使用ARIMA进行“单步”预测，并进行预测效果对比
...
%% 2.调用函数并实现单步预测
TrainR = 0.85; %用于训练的数据长度，也可以写成0.85
max_ar = 3;  %p上限值
max_ma = 3;  %q上限值
[forData,dataTrain,dataTest,aicorbic,res] = Fun_ARIMA_Forecast_Onestep(data,TrainR,max_ar,max_ma,'on','aic+bic');
% 进行使用ARIMA进行“单步”预测的函数，可以直接调用，差分阶数自动确定。
% 输入：
...
% 2.公开版代码训练集比例TrainR限定为0.85；      完整版无限制。
